...
impl Database {
    ...
    pub fn create_playlist(
        ...
        self.conn.execute(
            "INSERT INTO custom_playlists (id, name, description, image_url, created_at, updated_at, track_count) 
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, 0)",
            params![id, name, description, image_url, now, now],
        )?;
    ...
    pub fn get_all_playlists(&self) -> Result<Vec<CustomPlaylist>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, name, description, image_url, created_at, updated_at, track_count 
